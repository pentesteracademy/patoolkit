...
local function init_listener()
    ...
    function tap.packet(pinfo, tvb)
      ...
      for k,v in ipairs(type)
        ...
        local function get_dns(win,stringToFind)
            ...
            if(util.searchStr({v["domain"],v["ip"]},stringToFind))
                then
                  ...
                  win:append(dns_acf(dns_acf_settings,"|"))  
                end
          end
        ...
        register_menu("DNS/Resolution",dns_menu1, MENU_TOOLS_UNSORTED)

  init_listener()

end
        function dns_acf(settings,column_seperator)
          local final=""
          while(dns_isNext(settings))do
              for k,v in ipairs(settings)do
                  if(v["next"]==false) then v["value"]="" else v["next"]=false end
                  final=final..column_seperator..dns_format_str(v)
        ...
        function dns_format_str(global,substr)
            local m=0
            local n=0
            local str=""
            ...
            elseif(str:len()>len)
                ...
                global["next"]=true
                return dns_format_str(global,str:sub(1,a-1))
            end
            return s
        end
