...
local function init_listener()
    ...
    function tap.packet(pinfo, tvb)
        ...
        if pr_post()
          then
            local uri=pr_check2(request_uri())
            local req={}

            -- store the host, useragent and path
            req["host"]=pr_check2(host())
            req["user_agent"]=pr_check2(user_agent())
            req["path"]=uri
            ...
            for k,v in ipairs(key)do
              req["param"]=req["param"]..pr_check2(v) .. ": " .. pr_check2(value[k])..","
            end
        ...
        local function get_request(win,stringToFind)
            ...
            if(util.searchStr({v["host"],v["user_agent"],v["path"],v["param"]},stringToFind))
                then
                  ...
                  win:append(pr_acf(pr_acf_settings,"|"))  
                end
          end
        ...
        register_menu("Web/POST Requests",pr_menu1, MENU_TOOLS_UNSORTED)
  ...
  init_listener()
        ...
        function pr_acf(settings,column_seperator)
          local final=""
          while(pr_isNext(settings))do
              for k,v in ipairs(settings)do
                  if(v["next"]==false) then v["value"]="" else v["next"]=false end
                  final=final..column_seperator..pr_format_str(v)
        ...
        function pr_format_str(global,substr)
            local m=0
            local n=0
            local str=""
            ...
            elseif(str:len()>len)
                ...
                global["next"]=true
                return pr_format_str(global,str:sub(1,a-1))
            end
            return s
        end
