...
local function dialog_menu1(win,stringToFind)
		...
		do
			...
			win:append(arp_acf(arp_acf_settings,"|"))
 ...
 function arp_menu1()
  ...
  register_menu("ARP/ARP Table",arp_menu1, MENU_TOOLS_UNSORTED)
...
function arp_acf(settings,column_seperator)
  local final=""
  while(arp_isNext(settings))do
      for k,v in ipairs(settings)do
          if(v["next"]==false) then v["value"]="" else v["next"]=false end
          final=final..column_seperator..arp_format_str(v)
...
function arp_format_str(global,substr)
    local m=0
    local n=0
    local str=""
    ...
    elseif(str:len()>len)
        ...
        global["next"]=true
        return arp_format_str(global,str:sub(1,a-1))
    end
    return s
end
